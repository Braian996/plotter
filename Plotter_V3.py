...
class Main(wx.Frame):
	...
	def InitUI(self):
		...
		archivo_OP = wx.Menu() #Se crea opciones de menu
		...
		self.Bind(wx.EVT_MENU, self.OnQuit, salir_OP) #Evento para cerrar ventana
		...
		self.panel.Bind(wx.EVT_BUTTON, self.OnLeftX15, self.btnX_left10)
		self.panel.Bind(wx.EVT_BUTTON, self.OnLeftX20, self.btnX_left15)
		self.panel.Bind(wx.EVT_BUTTON, self.OnLeftX25, self.btnX_left20)

		self.panel.Bind(wx.EVT_BUTTON, self.OnRightX15, self.btnX_right10)
		self.panel.Bind(wx.EVT_BUTTON, self.OnRightX20, self.btnX_right15)
		self.panel.Bind(wx.EVT_BUTTON, self.OnRightX25, self.btnX_right20)

		self.panel.Bind(wx.EVT_BUTTON, self.OnLeftY15, self.btnY_left10)
		self.panel.Bind(wx.EVT_BUTTON, self.OnLeftY20, self.btnY_left15)
		self.panel.Bind(wx.EVT_BUTTON, self.OnLeftY25, self.btnY_left20)

		self.panel.Bind(wx.EVT_BUTTON, self.OnRightY15, self.btnY_right10)
		self.panel.Bind(wx.EVT_BUTTON, self.OnRightY20, self.btnY_right15)
		self.panel.Bind(wx.EVT_BUTTON, self.OnRightY25, self.btnY_right20)
	...
	def OnMove(self, event):
		...
		pm1 = 0 #Movimiento de MOTOR X
		pm2 = 0 #Movimiento de MOTOR Y
		DirMX = 0
		DirMY = 0
		...
		def convertir_num_decimal_string(numero):
			...
			if cont == tamano:	#Veo, si son iguales, no hay coma
				...
				return str(jojo)
		def secuenciaPasos(cuchilla, pasosx, pasosy):
			contX = 0
			contY = 0
		...
		for renglon in archivo:
			...
			if palabras[0] == 'G00':
				...
				if palabras[1][0] == 'X' and palabras[2][0] == 'Y':
					...
					cuchilla = 1
					print "G00 MOVIMIENTO"

					if float(movX0) != motorActualX:
						XD1 = float(movX0) - motorActualX
					if float(movY0) != motorActualY:
						YD1 = float(movY0) - motorActualY
					...
					pasosStrX = convertir_num_decimal_string(str(XD1/pasosMX)) #Se usa la funcion para corregir error de perdida de pasos
					pasosxG0 = int(pasosStrX) #Se convierte el valor a entero
					if pasosxG0 < 0: #Se convierte valores negativos de pasos a positivos
						pasosxG0 = pasosxG0 * -1

					pasosStrY = convertir_num_decimal_string(str(YD1/pasosMY))
					pasosyG0 = int(pasosStrY)
					...
					if pasosxG0 > 0 or pasosyG0 > 0:
						varSec = secuenciaPasos(cuchilla, pasosxG0, pasosyG0)
				...
				if palabras[1][0] == 'X' and palabras[2][0] == 'Y':
					...
					if pasosx > 0 or pasosy > 0:
						varSec = secuenciaPasos(cuchilla, pasosx, pasosy)
...
Main(None, 'Plotter V3')#Se llama al proceso principal
