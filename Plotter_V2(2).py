			...
			if palabras[0] == 'G00':
				...
				if palabras[1][0] == 'X' and palabras[2][0] == 'Y':
					...
					if pasosx > 0 or pasosy > 0:
						...
						#X SUPERA A Y
						if pasosx > pasosy:
							if pasosy >= 0 and pasosy <= 1:
								print "MOVIMIENTO SOLO X"
								for f in range(1,pasosx+1): #Se hace el ciclo de movimiento
									ser.write(chr(int(str('00000000'),2))) #Se le envia 0 al plotter
									time.sleep(tiempo01) #Se pone tiempo de espera
									h = '00001'+str(DirMX)+'00' #Se crea los 8 bit
							...
							#Y SUPERA A X
							if pasosy > pasosx:
								if pasosx >= 0 and pasosx <= 1:
									print "MOVIMIENTO SOLO Y"
									for f in range(1,pasosy+1): #Se hace el ciclo de movimiento
										ser.write(chr(int(str('00000000'),2))) #Se le envia 0 al plotter
										time.sleep(tiempo02) #Se pone tiempo de espera
										h = '0000001'+str(DirMY) #Se crea los 8 bit
										j = int(str(h),2) #Se los pasa a int elevado a 2
										ser.write(chr(j)) #Se lo envia en formato char
										time.sleep(tiempo02)	#Se pone tiempo de espera
								else:
									...
									while(yp>0):
										...
										if (cub_cont < cub_int):
											...
											time.sleep(tiempo01)
										yp = yp - 1	
							else:#MOVIMIENTO DE AMBOS MOTORES A LA PAR
								print "SI AMBOS MOTORES IGUALES"
